...
async def check_1h_reminders(db: AsyncSession, season: Season) -> None:
    ...
    if season.state == SeasonState.submit and season.submit_deadline:
        deadline = season.submit_deadline
        if timedelta(0) < deadline - now <= window and not season.submit_1h_reminder_sent:
            stragglers = await crud.users_who_havent_submitted(db, season.id)
            emails = [u.email for u in stragglers if u.email and u.email_notifications]
            await notify.send_urgent_reminder(
                emails,
                season.name,
                "Book submission",
                deadline.strftime("%a %b %d at %H:%M UTC"),
        ...
        deadline = season.ranking_deadline
        if timedelta(0) < deadline - now <= window and not season.ranking_1h_reminder_sent:
            stragglers = await crud.users_who_havent_ranked(db, season.id)
            emails = [u.email for u in stragglers if u.email and u.email_notifications]
            await notify.send_urgent_reminder(
                emails,
        ...
        current_round = await crud.get_current_bracket_round(db, season.id)
        if deadline and current_round and timedelta(0) < deadline - now <= window:
            if season.bracket_1h_reminder_round != current_round:
                stragglers = await crud.users_who_havent_voted_round(db, season.id, current_round)
                emails = [u.email for u in stragglers if u.email and u.email_notifications]
                await notify.send_urgent_reminder(
                    emails,
                ...
                season.bracket_1h_reminder_round = current_round
                await db.commit()


async def check_meetup_1h_reminder(db: AsyncSession, meetup: "Meetup") -> None:  # noqa: F821
    """Fire a 1-hour urgent reminder before the meetup poll closes."""
    now = datetime.utcnow()
    window = timedelta(hours=1)
    if not (timedelta(0) < meetup.deadline - now <= window) or meetup.reminder_1h_sent:
        return
    from sqlalchemy import select as sa_select
