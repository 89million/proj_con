...
def _seed(book_id: int) -> MagicMock:
    s = MagicMock()
    s.book_id = book_id
    return s
...
def test_seed_tiebreakers_tie_broken_by_submission_order():
    ...
    noms = {1: 2, 2: 2}
    result = seed_tiebreakers(seeds, scores, noms)
    assert result[1] == "submission_order"
    assert result[2] == "submission_order"


def test_seed_tiebreakers_tie_no_noms_submission_order():
    """Equal scores, no prior nominations → falls back to 'submission_order'."""
    seeds = [_seed(10), _seed(20)]
    scores = {10: 6, 20: 6}
    result = seed_tiebreakers(seeds, scores, {})
    assert result[10] == "submission_order"
    assert result[20] == "submission_order"
...
def test_seed_tiebreakers_partial_tie():
    """Three books: two tied, one clear — only tied ones get a tiebreaker label."""
    seeds = [_seed(1), _seed(2), _seed(3)]
    scores = {1: 8, 2: 8, 3: 2}
    result = seed_tiebreakers(seeds, scores, {})
    assert result[1] == "submission_order"
