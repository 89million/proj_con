...
# ---------------------------------------------------------------------------
# 1-hour urgent reminders — submit
# ---------------------------------------------------------------------------


@pytest_asyncio.fixture
async def submit_season_1h(db, test_admin, test_user):
    """Submit-phase season with a deadline 45 minutes away; only admin has submitted."""
    season = Season(
        name="1h Submit Test",
        state=SeasonState.submit,
        page_limit=400,
        submit_deadline=datetime.utcnow() + timedelta(minutes=45),
    )
    db.add(season)
    await db.commit()
    await db.refresh(season)
...
async def test_submit_1h_reminder_fires_within_1h(db, submit_season_1h):
    """check_1h_reminders sends urgent email to non-submitters when deadline < 1h."""
    season = submit_season_1h
    with patch("app.notify.notify_all", new_callable=AsyncMock) as mock_notify:
        await state.check_1h_reminders(db, season)
    mock_notify.assert_called_once()
    msg = mock_notify.call_args.kwargs.get("discord_msg", mock_notify.call_args.args[1])
    assert "1 hour" in msg or "🚨" in msg


async def test_submit_1h_reminder_not_sent_twice(db, submit_season_1h):
    """1h reminder not re-sent once flag is set."""
    season = submit_season_1h
    season.submit_1h_reminder_sent = True
    await db.commit()
    with patch("app.notify.notify_all", new_callable=AsyncMock) as mock_notify:
        await state.check_1h_reminders(db, season)
    mock_notify.assert_not_called()
...
async def test_submit_1h_reminder_not_fired_outside_window(db, test_admin):
    """No 1h reminder if deadline is more than 1h away."""
    season = Season(
        name="1h Far",
        state=SeasonState.submit,
        page_limit=400,
        submit_deadline=datetime.utcnow() + timedelta(hours=3),
    )
    db.add(season)
    await db.commit()
    await db.refresh(season)
...
# ---------------------------------------------------------------------------
# 1-hour urgent reminders — ranking
# ---------------------------------------------------------------------------


@pytest_asyncio.fixture
async def ranking_season_1h(db, test_admin, test_user):
    """Ranking-phase season with a deadline 45 minutes away; only admin has ranked."""
    season = Season(
        name="1h Ranking Test",
        state=SeasonState.ranking,
        page_limit=400,
        ranking_deadline=datetime.utcnow() + timedelta(minutes=45),
    )
    db.add(season)
    await db.commit()
    await db.refresh(season)
...
async def test_ranking_1h_reminder_fires_within_1h(db, ranking_season_1h):
    """check_1h_reminders sends urgent ranking reminder within 1h window."""
    season = ranking_season_1h
    with patch("app.notify.notify_all", new_callable=AsyncMock) as mock_notify:
        await state.check_1h_reminders(db, season)
    mock_notify.assert_called_once()
    msg = mock_notify.call_args.kwargs.get("discord_msg", mock_notify.call_args.args[1])
...
async def test_ranking_1h_reminder_not_sent_twice(db, ranking_season_1h):
    """1h ranking reminder not re-sent once flag is set."""
    season = ranking_season_1h
    season.ranking_1h_reminder_sent = True
    await db.commit()
    with patch("app.notify.notify_all", new_callable=AsyncMock) as mock_notify:
        await state.check_1h_reminders(db, season)
    mock_notify.assert_not_called()
...
# ---------------------------------------------------------------------------
# 1-hour urgent reminders — meetup
# ---------------------------------------------------------------------------


@pytest_asyncio.fixture
async def meetup_1h(db, test_admin, test_user):
    """A meetup poll closing in 45 minutes; only admin has voted."""
    season = Season(name="1h Meetup Season", state=SeasonState.complete, page_limit=400)
    db.add(season)
    await db.flush()
    db.add(SeasonParticipant(season_id=season.id, user_id=test_admin.id))
    db.add(SeasonParticipant(season_id=season.id, user_id=test_user.id))
    meetup = Meetup(
        season_id=season.id,
        deadline=datetime.utcnow() + timedelta(minutes=45),
    )
    db.add(meetup)
    await db.flush()
    option = MeetupOption(
...
async def test_meetup_1h_reminder_fires_within_1h(db, meetup_1h):
    """check_meetup_1h_reminder fires when poll closes within 1h."""
    meetup = meetup_1h
    with patch("app.notify.notify_all", new_callable=AsyncMock) as mock_notify:
...
async def test_meetup_1h_reminder_not_sent_twice(db, meetup_1h):
    """1h meetup reminder not re-sent once flag is set."""
    meetup = meetup_1h
...
async def test_meetup_1h_reminder_not_fired_outside_window(db, test_admin):
    """No 1h meetup reminder when deadline is more than 1h away."""
    season = Season(name="1h Far Meetup", state=SeasonState.complete, page_limit=400)
    db.add(season)
    await db.flush()
    db.add(SeasonParticipant(season_id=season.id, user_id=test_admin.id))
    meetup = Meetup(
